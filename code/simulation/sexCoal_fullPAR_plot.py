...
def sexCoal_2deme_fullPAR(chromSamp=BlackSamp, r=1, rho=1, Ne=1, num_replicates=10**4, mu=mu):
...
# Ne_smooth_r
# Ne_real_r
def sexCoal_2deme_fullPAR_makePlotData(chromSamp=BlackSamp, rVals=real_rate_map_cM_Mb, rhoVals=rhoVals, NeVals=Ne_real_r, num_replicates=10**4, mu=mu):
    # storage
    ...
    for i in rhoValsToRun:
        if rhoVals[i] == 0:
            count = count + 1
            continue
        res = sexCoal_2deme_fullPAR(num_replicates=num_replicates, r=rVals[i], rho=rhoVals[i], Ne=NeVals[i])
        Ts[i,0:36] = res[0:36]
