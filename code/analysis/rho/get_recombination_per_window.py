...
print("window"+"\t"+"Window_start"+"\t"+"Window_end"+"\t"+"chr"+"\t"+"chr_start"+"\t"+"chr_end"+"\t"+"scaffold"+"\t"+"scaf_start"+"\t"+"scaf_end"+"\t"+"rho_per_bp"+"\t"+"rho_per_window")
for key in f2_l: # for each window in the order of the sliding window list
	if len(f1_dict[key]) == 1: # if there is only 1 occurrence of that window
		...
		if f1_dict[key][0][0] == ".": # if there is no overlap
			print(key.split(":")[0]+"\t"+key.split(":")[1]+"\t"+key.split(":")[2]+"\t"+"NA"+"\t"+"NA"+"\t"+"NA"+"\t"+"NA"+"\t"+"NA"+"\t"+"NA"+"\t"+"NA"+"\t"+"NA")
		else: # if there is an overlap
			rho_per_site = float(f1_dict[key][0][6])/(int(f1_dict[key][0][5])-int(f1_dict[key][0][4]))
			window_size = int(key.split(":")[2]) - int(key.split(":")[1])
			print(key.split(":")[0]+"\t"+key.split(":")[1]+"\t"+key.split(":")[2]+"\t"+"\t".join(f1_dict[key][0][0:6])+"\t"+str(round(rho_per_site, 6))+"\t"+str(round(rho_per_site*window_size, 6)))
		...
		for element in f1_dict[key]:
			# print(element)
			rho_per_site = float(element[6])/(int(element[5])-int(element[4]))
			addup_pair_rho = addup_pair_rho + (int(element[10])*rho_per_site)
			total_pair = total_pair + int(element[10])
		new_pair_rho = addup_pair_rho/total_pair
		window_size = int(key.split(":")[2]) - int(key.split(":")[1])
		print(key.split(":")[0]+"\t"+key.split(":")[1]+"\t"+key.split(":")[2]+"\t"+element[0]+"\t"+element[1]+"\t"+element[2]+"\t"+element[3]+"\t"+element[4]+"\t"+element[5]+"\t"+str(round(new_pair_rho,6))+"\t"+str(round(new_pair_rho*window_size, 6)))
