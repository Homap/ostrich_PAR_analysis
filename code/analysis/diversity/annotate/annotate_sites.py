	...
	if strand == "+":
		for index, item in enumerate(gff_dict[key]):
			...
			if phase_cds == 0:
				...
				if not first_codon in stop_codons and 'N' not in first_codon:
				...
				second_part_codon = gff_dict[key][index][0]
				#print(seqid, key, first_part_codon, second_part_codon)
...
stop_transcripts_out = open("stop_transcripts.txt", "w")
for transcript in stop_transcripts:
	stop_transcripts_out.write(str(transcript)+"\n")
#*******************************************************************
# Close inputs
fastafile.close()
gff_file.close()
