...
#***********************************************************************
# Usage: ./recombination.py ../data/linkage_map/LGZ3.sex_averaged.lifted.bed
#***********************************************************************
...
for index, item in enumerate(pos_l):
	if index == 0:
		...
		if pos_l[index].split(":")[0] == pos_l[index-1].split(":")[0]: # Calculate the recombinate rate only within scaffolds.
			# This condition is to check that we are still in the same scaffold. If we change scaffold,
			# then we will not calculate recombination rate between a position in scaffold 1 and 2.
			# print(abs(float(pos_l[index-1].split(":")[2])-float(pos_l[index].split(":")[2])))
			# print(abs(int(pos_l[index-1].split(":")[1])-int(pos_l[index].split(":")[1])))
